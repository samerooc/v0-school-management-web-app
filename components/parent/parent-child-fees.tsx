...
interface ParentChildFeesProps {
  studentId: string
}

export default async function ParentChildFees({ studentId }: ParentChildFeesProps) {
                ...
                <TableBody>
                  {fees.map((fee) => (
                    <TableRow key={fee.id}>
                      ...
                      <TableCell className="text-right font-semibold">${Number(fee.amount).toFixed(2)}</TableCell>
